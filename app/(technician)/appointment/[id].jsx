...
import {
  ...
  selectReportIdsByAppointment,
  selectReportLoadingByAppointment,
...
export default function AppointmentDetailsScreen() {
  ...
  const invoicesError = useAppSelector((state) =>
    repairRequestId
      ? selectInvoicesErrorByRepairRequest(state, repairRequestId)
      : null
  );

  const completing = useAppSelector((state) =>
  ...
  const canCompleteMaintenanceAppointment = isMaintenance && allMaintenanceHaveStatus;
  ...
  const {
    ...
    const previousCount = idx; // số buổi trước (buổi hiện tại là idx+1)
    return {
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      promises.push(
        dispatch(fetchInspectionReportByAppointmentId(id)).unwrap()
      );
      promises.push(
        dispatch(
          fetchRepairReportByAppointment({ appointmentId: id })
        ).unwrap()
      );
  ...
  if (!initialLoaded) {
    return (
      ...
      >
        ...
        <Text style={{ marginTop: 8, color: zincColors[600] }}>
  ...
  const handleCreateInspectionReport = () => {
    ...
    const rrId = appointment?.repairRequest?.repairRequestId;
    if (!rrId) {
  ...
  const handleCreateInvoice = () => {
    ...
    router.push({
      ...
      params: {
        repairRequestId: Number(appointment?.repairRequest?.repairRequestId),
        solutionType: solutionTypeNorm || '',
        faultOwner: faultOwnerNorm || '',
        scenarioCode: scenarioCode ? String(scenarioCode) : '',
      },
    });
  };
    ...
    router.push({
      ...
      params: {
        ...
        repairRequestId: String(
          appointment?.repairRequest?.repairRequestId ?? ''
        ),
      },
    });
  };
  ...
  const handleCreateChatWithResident = async () => {
    const residentId =
      appointment?.appointmentTrackings?.[0]?.updatedByUser?.userId;
  ...
  const goInspectionReportDetail = (reportId) => {
    router.push({
      pathname: '/(technician)/inspectionReport/[id]',
  ...
  const goRepairReportDetail = (reportId) => {
    router.push({
      pathname: '/(technician)/repairReport/[id]',
      params: { id: String(reportId) },
    });
  };
  ...
  function normalizeTrackings(appointment) {
    ...
    return arr
      .map((x) => ({
        ...
        at: x.updatedAt,
        by: x.updatedByUser
          ? `${x.updatedByUser.firstName || ''} ${
              x.updatedByUser.lastName || ''
            }`.trim()
          : '',
      }))
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={2}>
          {capitalizeFirst(appointment?.repairRequest?.object) ||
            '-Tên cuộc hẹn-'}
        </Text>
          ...
          <View style={styles.sectionWrap}>
            <View style={styles.section}>
              ...
              <Text style={styles.description}>
                {capitalizeFirst(
                  appointment?.repairRequest?.description
                ) || '-'}
              </Text>
            </View>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.infoBlock}>
                  ...
                  value={
                    inspectionInfo
                      ? OWNER_LABEL[inspectionInfo.faultOwner] ??
                        String(inspectionInfo.faultOwner)
                      : '-'
                  }
                />
                )}
                <Item
                  ...
                  value={
                    inspectionInfo
                      ? SOLUTION_LABEL[inspectionInfo?.solutionType] ??
                        String(inspectionInfo?.solutionType)
                      : '-'
                  }
                />
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Báo giá / Hoá đơn</Text>
                    ...
                    {trackings.length === 0 ? (
                      ...
                      trackings.map((t) => (
                        <TimelineItem
                          ...
                          icon="clock"
                          title={
                            STATUS_DISPLAY[t.status] || t.status
                          }
          ...
          <View style={styles.sectionWrap}>
            <View style={styles.chatPlaceholder}>
              ...
              <Pressable
                ...
                onPress={handleCreateChatWithResident}
              >
        ...
        {appointment?.status === 'InRepair' && (
          <>
            <Pressable
              ...
              onPress={handleCreateRepairReport}
            >
              <Icon name="pencil" size={20} color={appleBlue} />
              <Text style={styles.primaryBtnText}>
                {hasRepairReport
                  ? 'Thêm báo cáo sửa chữa'
                  : 'Báo cáo sửa chữa'}
              </Text>
            </Pressable>
