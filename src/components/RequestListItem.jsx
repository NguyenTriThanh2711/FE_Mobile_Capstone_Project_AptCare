...
const C = {
  ...
  emBg: "#FEE2E2", emFg: "#B91C1C",
  nmBg: "#E6F0FF", nmFg: "#0C4A6E",

  // status chips
  pdBg: "#F4F6F8", pdFg: "#475569", pdBd: "#E5E7EB",
  asBg: "#EEF7FF", asFg: "#0A66C2", asBd: "#BBD7FF",
  wkBg: "#FFF7ED", wkFg: "#C2410C", wkBd: "#FED7AA",
  cmBg: "#EAFBE7", cmFg: "#2E7D32", cmBd: "#B7E4B0",
  ccBg: "#FEF2F2", ccFg: "#B91C1C", ccBd: "#FECACA",
};
...
export default function RequestListItem({ item }) {
  ...
  const goToDetail = () => {
    ...
    router.push({ pathname: "/(resident)/request/[id]", params: { id: String(item?.repairRequestId) } });
      ...
      <Pressable onPress={goToDetail} style={s.card}>
        ...
        {!!item?.issue?.name && (
          <View style={s.issueRow}>
            ...
            <Text style={s.issueTxt} numberOfLines={1}>Vấn đề: {item.issue.name}</Text>
          </View>
        )}
        ...
        <View style={s.metaRow}>
          ...
          <View style={s.metaItem}>
            ...
            <Text style={[s.metaTxt, s.metaStrong]}>Tầng {floorLabel} • P.{roomLabel}</Text>
          </View>
        </View>
        {startAppointment && (
          <View style={[s.metaRow, { marginTop: 6 }]}>
            <View style={s.metaItem}>
              ...
              <Text style={[s.metaTxt, s.arrivalTxt]}>K/Thuật viên tới: {fmtDateTime(startAppointment)}</Text>
            </View>
          </View>
        )}
      </Pressable>
