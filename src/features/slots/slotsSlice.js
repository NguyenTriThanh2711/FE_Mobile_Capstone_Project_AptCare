...
export const fetchSlots = createAsyncThunk(
  ...
  async(_payload, { rejectWithValue }) => {
    try {
      ...
      const message = err?.response?.data?.detail || err?.response?.data?.message || "Không tải được danh sách ca làm việc";
      console.log(err)
...
export const selectSlotById = (state, slotId) => state.slots?.mapById ? state.slots.mapById[slotId] : null;
